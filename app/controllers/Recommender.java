...
public class Recommender {
	...
	public ArrayList<String> run(Integer targetUser){
		...
		HashMap<Short, HashMap<Location, Double>> preCompU = initTargetUserLocs(allUsers.get(targetUser));
		for(int z=0; z < recScoreForLocsTemporal.length; z++){	
			...
			for (Map.Entry<Integer, HashMap<Location, HashSet<Short>>> entry : allUsers.entrySet()){
				double magnitude = 0;
				if (entry.getValue().containsKey(allLocs.get(z))){
					double similarity = cosineSimilarityBetweenUsers(preCompU, allUsers.get(targetUser), entry.getValue());
		...
		for (int i=sortedScores.size()-1; i >= sortedScores.size() - 10; i-- ){
			...
			output.add(String.valueOf(normalizedScore));
		}*/
		return output;
	}
	public static HashMap<Short, HashMap<Location, Double>> initTargetUserLocs(HashMap<Location, HashSet<Short>> arr){
		HashMap<Short, HashMap<Location, Double>> output = new HashMap<Short, HashMap<Location, Double>>();
		for (Short i=0; i < 24; i++){
			HashMap<Location, Double> locVal = new HashMap<Location, Double>();
			for(Map.Entry<Location, HashSet<Short>> entry : arr.entrySet()){
				double val = computeNewValue(arr, entry.getKey(), i);
				locVal.put(entry.getKey(), val);
			}
			output.put(i, locVal);
		}
		return output;
	...
	public static double cosineSimilarityBetweenUsers(HashMap<Short, HashMap<Location, Double>> preCompU, HashMap<Location, HashSet<Short>> userU,HashMap<Location, HashSet<Short>> userV){
		double cU, cV;
		double dotProduct = 0;
		double magnitude1 = 0;
		double magnitude2 = 0;
		...
		for (Short j=0; j < 24; j++){
			...
			while (itr.hasNext()){
				Location l = itr.next();
				cU = (userU.containsKey(l)) ? preCompU.get(j).get(l).doubleValue() : 0;
				cV = (userV.containsKey(l)) ? computeNewValue(userV, l, j) : 0;
				dotProduct += cU * cV;
	...
	public static double computeNewValue(HashMap<Location, HashSet<Short>> user, Location loc, Short atTime){
		...
		return numerator / denominator; //denominator never equals to 0
	}
